...
YOSYS_FLAGS  = -DICE40 -DSYNTH_YOSYS
...
$(BIN_DIR)/%.asc: $(BUILD_DIR)/%.json
	...
	@if echo " $(ICE40_FIND_SEED_MODULES) " | grep -q " $(*) "; then \
		$(MAKE) $(BIN_DIR)/$(*).seed && \
		{ $(NEXTPNR) --seed $$(cat $(BIN_DIR)/$(*).seed) --asc $(@) 2>&1 | tee $(@).log; test $${PIPESTATUS[0]} -eq 0 || { echo ""; echo "NEXTPNR FAILED. To reproduce:"; echo "  $(NEXTPNR) --seed $$(cat $(BIN_DIR)/$(*).seed) --asc $(@)"; echo ""; rm -f $(@); exit 1; }; }; \
	else \
		{ $(NEXTPNR) --asc $(@) 2>&1 | tee $(@).log; test $${PIPESTATUS[0]} -eq 0 || { echo ""; echo "NEXTPNR FAILED. To reproduce:"; echo "  $(NEXTPNR) --asc $(@)"; echo ""; rm -f $(@); exit 1; }; }; \
	fi
