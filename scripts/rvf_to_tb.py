...
def extract_from_vcd(vcd_file):
    ...
    #
    # Extract spec assertions at times when spec_valid=1 (instruction checks)
    #
    ...
    #
    # Extract reg check assertions at times when check=1 (register checks)
    #
    check_signal = checker_signals.get("check", [])
    reg_checks = []
    for t, check_val in check_signal:
        if check_val == "1":
            order_val = get_value_at_time(rvfi_order, t)
            if order_val and "x" not in order_val.lower():
                order = int(order_val, 2)
            ...
            reg_check = {"order": order}

            #
            # Get register index being checked
            #
            reg_idx_tv = checker_signals.get("register_index", [])
            reg_idx_val = get_value_at_time(reg_idx_tv, t)
            if reg_idx_val and "x" not in reg_idx_val.lower():
                reg_check["register_index"] = int(reg_idx_val, 2)

            #
            # Get expected shadow value
            #
            shadow_tv = checker_signals.get("register_shadow", [])
            shadow_val = get_value_at_time(shadow_tv, t)
            if shadow_val and "x" not in shadow_val.lower():
                reg_check["register_shadow"] = int(shadow_val, 2)

            #
            # Get rs1/rs2 addresses and data
            #
            for sig in ["rvfi_rs1_addr", "rvfi_rs2_addr", "rvfi_rs1_rdata", "rvfi_rs2_rdata"]:
                tv = checker_signals.get(sig, [])
                val = get_value_at_time(tv, t)
                if val and "x" not in val.lower():
                    reg_check[sig] = int(val, 2)

            reg_checks.append(reg_check)

    #
    # Extract liveness check info (insn_order anyconst)
    #
    insn_order_tv = checker_signals.get("insn_order", [])
    liveness_insn_order = None
    if insn_order_tv:
        #
        # insn_order is an anyconst, so it's constant throughout the trace
        #
        val = insn_order_tv[0][1]
        if val and "x" not in val.lower():
            liveness_insn_order = int(val, 2)

    return prog, dmem, spec_checks, reg_checks, liveness_insn_order


def generate_testbench(
    prog, dmem_per_cycle, spec_checks, reg_checks, init_state, name, max_cycles, params,
    formal_target, liveness_insn_order=None
):
    ...
    return template.render(
        ...
        liveness_insn_order=liveness_insn_order,
        **params,
    )
...
def main():
    ...
    #
    # Extract spec checks, reg checks, and liveness info from VCD
    #
    _, _, spec_checks, reg_checks, liveness_insn_order = extract_from_vcd(vcd_file)
    print(f"# Extracted {len(spec_checks)} spec checks from VCD", file=sys.stderr)
    print(f"# Extracted {len(reg_checks)} reg checks from VCD", file=sys.stderr)
    if liveness_insn_order is not None:
        print(f"# Extracted liveness insn_order={liveness_insn_order} from VCD", file=sys.stderr)
    ...
    tb = generate_testbench(
        ...
        params, formal_target, liveness_insn_order
    )
